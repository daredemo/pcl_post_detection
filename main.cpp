...
double addMultiplication(Eigen::VectorXf a, Eigen::VectorXf b)
{
    double sum = 0.0;
    for (int i = 0; i < 3; i++){
        sum += a[i]*b[i];
...
double angleBetweenPlaneVector(Eigen::VectorXf plane, Eigen::VectorXf vec)
{
    double a, b, c, d;
    a = addMultiplication(vec, plane);
    b = addMultiplication(vec, vec);
    c = addMultiplication(plane, plane);
    d = std::abs(a/(std::sqrt(b*c)));

    print_info("\nAngle: %g\n", std::abs(std::asin(d))/3.14*180.0);

    return std::abs(std::asin(d))/3.14*180.0;
}

bool angleIsLess(Eigen::VectorXf ground, Eigen::VectorXf plane, double angle)
{
    double a, b, c, d;
    a = addMultiplication(ground, plane);
    b = addMultiplication(ground, ground);
    c = addMultiplication(plane, plane);
    d = std::abs(a)/(std::sqrt(b*c));

    print_info("\nAngle: %g\n", std::abs(std::acos(d))/3.14*180.0);

    if(std::abs(std::acos(d))/3.14*180.0 < angle)
        return true;
    else
        return false;
}
...
main (int argc, char** argv)
{
  ...
  for (size_t i = 0; i < p_file_indices.size (); ++i)
  {
    ...
    seg.setRadiusLimits(0.0001, 1.75);
    seg.setMethodType (pcl::SAC_RANSAC); // from middle (always?) SAC_PROSAC 

    pcl::PointXYZ minPt, maxPt;
    pcl::getMinMax3D(*xyz, minPt, maxPt);
    print_value("\nINPUT:\nMIN/MAX:\nx: %g; %g\ny: %g; %g\nz: %g; %g\n", minPt.x, maxPt.x, minPt.y, maxPt.y, minPt.z, maxPt.z);

    print_value("MAX-MIN: %g\n", maxPt.z-minPt.z);

    double t = 0.75*(maxPt.z-minPt.z);

    seg.setDistanceThreshold(t); // (threshold);
    seg.setMaxIterations(1000); //(max_iterations);

    seg.setInputCloud (xyz);
    seg.segment (*inliers, *coefficients);

    for (const auto&i: coefficients->values){
      print_value("%lf\n", i);
    }
    ...
    pcl::compute3DCentroid(*xyz, *inliers, centroid);

    if (inliers->indices.size () == 0)
    {
        PCL_ERROR ("No STICK model found. Relax thresholds and continue.\n");
    }
    else
    {
      /* // FOR CYLINDERS
      ...
      cylinder_coeff.values[3] = coefficients->values[3] * t * 0.5;
      cylinder_coeff.values[4] = coefficients->values[4] * t * 0.5;
      cylinder_coeff.values[5] = coefficients->values[5] * t * 0.5;
      cylinder_coeff.values[6] = 0.25;
      std::string cylinder_name = "cylinder_";
      cylinder_name.append(std::to_string(i));
      p->addCylinder(cylinder_coeff, cylinder_name, 0);
      p->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_REPRESENTATION, pcl::visualization::PCL_VISUALIZER_REPRESENTATION_WIREFRAME, cylinder_name);
      cylinder_name.append("a");
      ...
      */
      ...
      pcl::PCLPointCloud2::Ptr output_rene (new pcl::PCLPointCloud2);
      pcl::copyPointCloud (*cloud, inliers->indices, *output_rene);

      Eigen::VectorXf ground(3);
      Eigen::VectorXf post(3);
      ground << 0., 0., 1.0;
      post << coefficients->values[3], coefficients->values[4], coefficients->values[5];

      if((angleBetweenPlaneVector(ground, post) > 80.0) && (inliers->indices.size () > 0.5*cloud->height*cloud->width)) // && (angleBetweenPlaneVector(ground, post) < 100.0))
      {
        pcl::MomentOfInertiaEstimation <pcl::PointXYZ> feature_extractor;
        feature_extractor.setInputCloud (xyz);
        ...
        std::string cube_name_AABB = "AABB";
        std::string cube_name_OBB = "OBB";
        std::string major_name = "major eigen vector ";
        std::string minor_name = "minor eigen vector ";
        std::string middle_name = "middle eigen vector ";
        cube_name_AABB.append(std::to_string(i));
        cube_name_OBB.append(std::to_string(i));
        major_name.append(std::to_string(i));
        minor_name.append(std::to_string(i));
        middle_name.append(std::to_string(i));

        p->addCube (min_point_AABB.x, max_point_AABB.x, min_point_AABB.y, max_point_AABB.y, min_point_AABB.z, max_point_AABB.z, 1.0, 1.0, 0.0, cube_name_AABB);
        p->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_REPRESENTATION, pcl::visualization::PCL_VISUALIZER_REPRESENTATION_WIREFRAME, cube_name_AABB);

        Eigen::Vector3f position (position_OBB.x, position_OBB.y, position_OBB.z);
        Eigen::Quaternionf quat (rotational_matrix_OBB);

        std::cout << "OBB.max.x: " << max_point_OBB.x << "; OBB.min.x: " << min_point_OBB.x << std::endl;
        std::cout << "OBB.max.y: " << max_point_OBB.y << "; OBB.min.y: " << min_point_OBB.y << std::endl;
        std::cout << "OBB.max.z: " << max_point_OBB.z << "; OBB.min.z: " << min_point_OBB.z << std::endl;

        double area = std::abs(4.0 * max_point_OBB.y * max_point_OBB.z);

        if (4 < std::abs(max_point_OBB.x - min_point_OBB.x) && 16.0 > std::abs(max_point_OBB.x - min_point_OBB.x) && 3.0 > area && 0.05 < area)
        {
          p->addCube (position, quat, max_point_OBB.x - min_point_OBB.x, max_point_OBB.y - min_point_OBB.y, max_point_OBB.z - min_point_OBB.z, cube_name_OBB);
          p->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_REPRESENTATION, pcl::visualization::PCL_VISUALIZER_REPRESENTATION_WIREFRAME, cube_name_OBB);
          p->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR, 1.0, 0.0, 0.0, cube_name_OBB);
          pcl::ModelCoefficients cylinder_coeff;
          cylinder_coeff.values.resize (7);    // We need 7 values
          cylinder_coeff.values[0] = coefficients->values[0];
          cylinder_coeff.values[1] = coefficients->values[1];
          ...
          ofstream out_posts;
          out_posts.open("posts.csv", ios::out | ios::app);
          out_posts << coefficients->values[0] << ", " << coefficients->values[1] << ", " << min_point_OBB.x << ", " << max_point_OBB.x - min_point_OBB.x << std::endl;
          out_posts.close();
        }
        else
        {
          PCL_ERROR("SIZE ERROR!\n");
        }
        ...
        p->addLine (center, x_axis, 1.0f, 0.0f, 0.0f, major_name);
        p->addLine (center, y_axis, 0.0f, 1.0f, 0.0f, middle_name);
        p->addLine (center, z_axis, 0.0f, 0.0f, 1.0f, minor_name);
      }
      else
      {
          PCL_ERROR("ANGLE ERROR!\n");
      }
    }
